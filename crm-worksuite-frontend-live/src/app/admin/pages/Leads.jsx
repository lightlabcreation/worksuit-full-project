...
const Leads = () => {
  ...
  // Fixed Lead Stages
  const leadStages = [
    { id: 'New', label: 'New', color: 'bg-blue-100 text-blue-800 border-blue-300' },
    { id: 'Qualified', label: 'Qualified', color: 'bg-purple-100 text-purple-800 border-purple-300' },
    { id: 'Discussion', label: 'Discussion', color: 'bg-yellow-100 text-yellow-800 border-yellow-300' },
    { id: 'Negotiation', label: 'Negotiation', color: 'bg-orange-100 text-orange-800 border-orange-300' },
    { id: 'Won', label: 'Won', color: 'bg-green-100 text-green-800 border-green-300' },
    { id: 'Lost', label: 'Lost', color: 'bg-red-100 text-red-800 border-red-300' },
  ]
  ...
  }, [activeFilter, searchQuery, probabilityFilter, activeTab, dateRange, companyId, currentMonth])
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (probabilityFilter) {
          filtered = filtered.filter(l => l.probability >= parseInt(probabilityFilter))
        }
  ...
  const getFilteredLeads = () => {
    ...
    // Apply main filter
    if (activeFilter === 'my') {
      filtered = filtered.filter(lead => lead.employee === loggedInUser)
      ...
      filtered = filtered.filter(lead => lead.source === 'Call')
    }
  ...
  const fetchLabels = async () => {
    try {
      ...
      if (response.data.success) {
        // Extract unique label names from the response
        const uniqueLabels = [...new Set((response.data.data || []).map(item => item.label))]
      ...
      if (labels.length === 0) {
        setLabels(['New', 'Hot', 'Cold', 'Follow-up', 'Converted'])
      }
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => (
        <div>
          ...
          {row.companyName && (
            <p className="text-xs text-secondary-text">{row.companyName}</p>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (value) => {
        ...
        return (
          <Badge
            variant="default"
            className={`text-xs ${stage?.color || 'bg-gray-100 text-gray-800'}`}
          >
            {value}
          </Badge>
        )
      ...
      render: (value, row) => (
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-full bg-primary-accent/20 flex items-center justify-center text-xs font-semibold text-primary-accent">
            {row.employeeAvatar || value.charAt(0)}
          </div>
    ...
    },
    {
      ...
      render: (value, row) => {
        ...
        const labelNames = Array.isArray(value) ? value : [value]
        return (
    ...
    return (
      <div className="flex items-center justify-end gap-1 sm:gap-2 flex-shrink-0">
        <button
          ...
          className="p-1.5 sm:p-2 text-green-600 bg-green-50 hover:bg-green-100 rounded-lg transition-colors flex-shrink-0"
          title="View"
          type="button"
        >
        ...
        <button
          ...
          className="p-1.5 sm:p-2 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors flex-shrink-0"
          title="Edit"
          type="button"
        >
        ...
        <button
          ...
          className="p-1.5 sm:p-2 text-purple-600 bg-purple-50 hover:bg-purple-100 rounded-lg transition-colors flex-shrink-0"
          title="Convert to Client"
          type="button"
        >
          <IoPersonAdd size={16} className="sm:w-[18px] sm:h-[18px]" />
        </button>
        <button
          onClick={(e) => {
            e.preventDefault()
          ...
          className="p-1.5 sm:p-2 text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors flex-shrink-0"
          title="Delete"
          type="button"
        >
    ...
    <div className="space-y-3 sm:space-y-4 md:space-y-6 w-full">
      ...
      <div className="bg-white rounded-lg shadow-sm p-3 sm:p-4 md:p-5 w-full">
        ...
        <div className="relative mb-3 sm:mb-4 w-full">
          <IoTimeOutline className="absolute left-3 top-1/2 transform -translate-y-1/2 text-secondary-text w-4 h-4 sm:w-5 sm:h-5" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Search leads..."
            className="w-full pl-9 sm:pl-10 pr-9 sm:pr-10 py-2 text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-accent focus:border-primary-accent outline-none"
          />
          <IoSearch className="absolute right-3 top-1/2 transform -translate-y-1/2 text-secondary-text w-4 h-4 sm:w-5 sm:h-5" />
        </div>

        {/* Filter and Action Bar */}
        <div className="flex flex-wrap items-center gap-2 overflow-x-hidden pb-2 w-full">
          <button
            onClick={() => window.location.reload()}
            className="p-1.5 sm:p-2 text-secondary-text hover:text-primary-text hover:bg-gray-100 rounded-lg transition-colors"
            title="Refresh"
          >
            <IoRefresh size={18} className="sm:w-5 sm:h-5" />
          </button>
          {selectedLeads.length > 0 && (
            <>
              <button
                ...
                className="px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-red-600 bg-white border border-red-300 rounded-lg hover:bg-red-600 hover:text-white transition-all duration-200 flex items-center gap-1.5 sm:gap-2 whitespace-nowrap"
              >
                <IoTrash size={14} className="sm:w-4 sm:h-4" />
                <span className="hidden sm:inline">Delete ({selectedLeads.length})</span>
                <span className="sm:hidden">Delete</span>
              </button>
              <button
                onClick={() => {
                  const newStatus = prompt('Enter new status (New, Qualified, Discussion, Negotiation, Won, Lost):')
                  if (newStatus) {
                    handleBulkAction('change_status', { status: newStatus })
                  }
                }}
                className="px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium text-blue-600 bg-white border border-blue-300 rounded-lg hover:bg-blue-600 hover:text-white transition-all duration-200 flex items-center gap-1.5 sm:gap-2 whitespace-nowrap"
              >
                <IoCreate size={14} className="sm:w-4 sm:h-4" />
          ...
          <button
            ...
            className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-all duration-200 whitespace-nowrap ${probabilityFilter === '50'
              ? 'bg-yellow-100 text-yellow-800'
              : 'bg-white text-secondary-text border border-gray-300 hover:bg-gray-800 hover:text-white hover:border-gray-800'
              }`}
          >
            50%
          </button>
          <button
            ...
            className={`px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm font-medium rounded-lg transition-all duration-200 whitespace-nowrap ${probabilityFilter === '90'
          ...
          <button
            ...
            className={`p-1.5 sm:p-2 rounded-lg transition-colors ${activeFilter === 'call'
              ? 'bg-blue-100 text-blue-600'
              : 'text-secondary-text hover:text-primary-text hover:bg-gray-100'
              }`}
            title="Lead Call"
          >
            <IoCall size={18} className="sm:w-5 sm:h-5" />
          </button>
          <button
            onClick={() => setActiveFilter(activeFilter === 'my' ? 'all' : 'my')}
            className={`p-1.5 sm:p-2 rounded-lg transition-colors ${activeFilter === 'my'
              ? 'bg-primary-accent/10 text-primary-accent'
              : 'text-secondary-text hover:text-primary-text hover:bg-gray-100'
              }`}
            title="My Leads"
          >
            <IoPerson size={18} className="sm:w-5 sm:h-5" />
          </button>
        </div>
      </div>

      {/* Quick Filters */}
      <div className="flex flex-wrap items-center gap-2 p-2 sm:p-3 bg-gray-50 rounded-lg border border-gray-200 overflow-x-hidden w-full -mx-3 sm:mx-0 px-3 sm:px-0">
        <button
          onClick={() => {
            setActiveFilter('all')
            setProbabilityFilter(null)
          }}
          className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap ${activeFilter === 'all' && !probabilityFilter
            ? 'bg-primary-accent text-white'
            : 'bg-white text-primary-text border border-gray-300 hover:bg-gray-800 hover:text-white hover:border-gray-800'
            }`}
        >
          All Leads
        </button>
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap ${activeFilter === 'my'
            ? 'bg-primary-accent text-white'
            : 'bg-white text-primary-text border border-gray-300 hover:bg-gray-800 hover:text-white hover:border-gray-800'
            }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap ${activeFilter === 'thisWeek'
            ? 'bg-primary-accent text-white'
            : 'bg-white text-primary-text border border-gray-300 hover:bg-gray-800 hover:text-white hover:border-gray-800'
            }`}
        >
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs sm:text-sm font-medium transition-all duration-200 whitespace-nowrap ${activeFilter === 'call'
            ? 'bg-primary-accent text-white'
            : 'bg-white text-primary-text border border-gray-300 hover:bg-gray-800 hover:text-white hover:border-gray-800'
            }`}
        >
        ...
        {selectedLead && (
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-2">
              <div>
                <h3 className="text-base sm:text-lg font-semibold text-primary-text">{selectedLead.personName}</h3>
                {selectedLead.companyName && (
                  <p className="text-xs sm:text-sm text-secondary-text mt-1">{selectedLead.companyName}</p>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button
                  ...
                  className="flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3"
                >
                  <IoMail size={14} className="sm:w-4 sm:h-4" />
                  <span className="hidden xs:inline">Email</span>
                </Button>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => handleCreateTask(selectedLead)}
                  className="flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3"
                >
                  <IoCheckbox size={14} className="sm:w-4 sm:h-4" />
                ...
                <Button
                  ...
                  className="flex items-center gap-1 text-xs sm:text-sm px-2 sm:px-3"
                >
                  <IoPersonAdd size={14} className="sm:w-4 sm:h-4" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Lead Type</label>
                <p className="text-sm sm:text-base text-primary-text mt-1">
                  <Badge variant="default">{selectedLead.leadType || '-'}</Badge>
                </p>
              </div>
              <div>
                ...
                <div className="mt-1">
                  <Badge
                    variant="default"
                    className={leadStages.find(s => s.id === selectedLead.status)?.color || ''}
                  >
                    {selectedLead.status || '-'}
                  </Badge>
                </div>
              ...
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Email</label>
                <p className="text-sm sm:text-base text-primary-text mt-1 break-all">{selectedLead.email}</p>
              </div>
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Phone</label>
                <p className="text-sm sm:text-base text-primary-text mt-1">{selectedLead.phone}</p>
              </div>
              {selectedLead.companyName && (
                <div>
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">Company</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">{selectedLead.companyName}</p>
                </div>
              )}
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Source</label>
                <p className="text-sm sm:text-base text-primary-text mt-1">
                  <Badge variant="default">{selectedLead.source || '-'}</Badge>
                </p>
              </div>
              {selectedLead.probability && (
                <div>
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">Probability</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">
                    <Badge variant={selectedLead.probability === 90 ? 'success' : 'warning'}>
                      {selectedLead.probability}%
                    </Badge>
                  </p>
                </div>
              )}
              {selectedLead.callThisWeek && (
                <div>
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">Call Status</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">
                    <Badge variant="info">Call this week</Badge>
                  </p>
                </div>
              )}
              {selectedLead.address && (
                <div className="col-span-2">
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">Address</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">{selectedLead.address}</p>
                </div>
              )}
              {selectedLead.city && (
                <div>
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">City</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">{selectedLead.city}</p>
                </div>
              )}
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Value</label>
                <p className="text-sm sm:text-base text-primary-text mt-1">${selectedLead.value?.toLocaleString() || 0}</p>
              </div>
              <div>
                <label className="text-xs sm:text-sm font-medium text-secondary-text">Employee</label>
                <div className="flex items-center gap-2 mt-1">
                  <div className="w-6 h-6 rounded-full bg-primary-accent/20 flex items-center justify-center text-xs font-semibold text-primary-accent">
                    {selectedLead.employeeAvatar}
                  </div>
                  <p className="text-sm sm:text-base text-primary-text">{selectedLead.employee}</p>
                </div>
              </div>
              {selectedLead.dueFollowup && (
                <div>
                  <label className="text-xs sm:text-sm font-medium text-secondary-text">Due Follow-up</label>
                  <p className="text-sm sm:text-base text-primary-text mt-1">
                    {new Date(selectedLead.dueFollowup).toLocaleDateString()}
                  </p>
                </div>
              )}
              <div>
                ...
                <p className="text-sm sm:text-base text-primary-text mt-1">
                  {new Date(selectedLead.createdDate).toLocaleDateString('en-GB', {
                    day: 'numeric',
                    month: 'short',
                    year: 'numeric',
                  })}
          ...
          <div className="flex flex-wrap gap-2 justify-center py-2">
            {[
              ...
              <button
                ...
                className={`w-6 h-6 rounded transition-all ${newLabelColor === color ? 'ring-2 ring-offset-2 ring-gray-400 scale-110' : 'hover:scale-110'}`}
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-primary-accent focus:border-primary-accent outline-none"
                style={{ borderLeftWidth: '4px', borderLeftColor: newLabelColor }}
              />
            </div>
            <Button
              variant="outline"
          ...
          <div className="flex flex-wrap gap-2 pt-2">
            {labels.map((labelItem) => {
              ...
              return (
                <div
                  key={labelName}
                  className="flex items-center gap-1 px-3 py-1.5 rounded text-white text-sm font-medium cursor-pointer hover:opacity-90 transition-opacity"
                  style={{ backgroundColor: labelColor }}
                  onClick={() => {
                    // Edit label - populate form
            ...
            {labels.length === 0 && (
              <p className="text-sm text-secondary-text">No labels added yet</p>
            )}
          </div>
