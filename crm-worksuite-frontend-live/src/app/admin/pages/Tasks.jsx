...
const Tasks = () => {
  ...
  const [labels, setLabels] = useState(['Bug', 'Design', 'Enhancement', 'Feedback'])
  ...
  useEffect(() => {
    if (isEditModalOpen && selectedTask) {
      setFormData({
        ...
        points: selectedTask.points?.toString() || '1',
        assign_to: selectedTask.assigned_to_id || selectedTask.assigned_to || '',
        collaborators: selectedTask.collaborators || [],
        status: selectedTask.status || 'Incomplete',
        priority: selectedTask.priority || 'Medium',
        labels: selectedTask.labels || selectedTask.tags || [],
  ...
  const getFilteredTasks = () => {
    ...
    if (activeFilter === 'My Tasks') {
      ...
      filtered = filtered.filter(task => task.labels.includes('Bug'))
    }
    ...
    if (filters.label.length > 0) {
      filtered = filtered.filter(task =>
        task.labels.some(label => filters.label.includes(label))
      )
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="border-t border-gray-200 pt-4 mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto border border-gray-300 rounded-lg p-2">
                  {labels.map(label => (
                    <label key={label} className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded cursor-pointer">
                      <input
                        type="checkbox"
                        checked={filters.label.includes(label)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setFilters({ ...filters, label: [...filters.label, label] })
                          } else {
                            setFilters({ ...filters, label: filters.label.filter(l => l !== label) })
                          }
                        }}
                        className="w-4 h-4 text-primary-accent rounded focus:ring-primary-accent"
                      />
                ...
                {loading ? (
                  ...
                  filteredTasks.map((task) => (
                    <tr key={task.id} className={`hover:bg-gray-50 ${task.borderColor || ''}`}>
                      ...
                      <td className="px-2 sm:px-4 py-3 text-xs sm:text-sm text-primary-text font-medium">
                        {task.id}
                      </td>
                      <td className="px-2 sm:px-4 py-3">
                          ...
                          {task.labels && task.labels.length > 0 && (
                            <div className="flex items-center gap-1 mt-1 flex-wrap">
                              ...
                              {task.labels.slice(0, 2).map((label, idx) => (
                                <Badge key={idx} className={`text-xs ${getLabelColor(label)} flex-shrink-0`}>
                                  {label}
                                </Badge>
                              ))}
                            </div>
                      ...
                      <td className="px-2 sm:px-4 py-3">
                        {task.labels && task.labels.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {task.labels.slice(0, 2).map((label, idx) => (
                              <Badge key={idx} className={`text-xs ${getLabelColor(label)}`}>
                                {label}
                              </Badge>
                            ))}
            ...
            <FormRow label="Labels">
              <div className="space-y-2">
                ...
                >
                  ...
                  {labels.filter(l => !(formData.labels || []).includes(l)).map(l => (
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-secondary-text">Manage task labels</p>
          <div className="space-y-2">
            {labels.map((label, idx) => (
              <div key={idx} className="flex items-center justify-between p-2 border border-gray-200 rounded-lg">
                <Badge className={getLabelColor(label)}>{label}</Badge>
                <button
                  onClick={() => {
