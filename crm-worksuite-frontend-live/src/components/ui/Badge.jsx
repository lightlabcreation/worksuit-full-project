...
const Badge = ({ children, variant = 'default', className = '' }) => {
  const variants = {
    ...
    success: 'bg-primary-accent bg-opacity-10 text-primary-accent',
    warning: 'bg-warning bg-opacity-10 text-warning',
    danger: 'bg-danger bg-opacity-10 text-danger',
    info: 'bg-secondary-accent bg-opacity-10 text-secondary-accent',
  }

  // Handle undefined/null/empty children gracefully
  const displayValue = children == null || children === '' ? '-' : String(children)
