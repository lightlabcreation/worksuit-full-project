    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="px-4 py-3 bg-gray-50 border-b border-gray-200">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <button
            ...
            className={`inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium border rounded-md transition-all ${showFilters
          ...
          <div className="relative" ref={columnToggleRef}>
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium border rounded-md transition-all ${showColumnToggle
                ? 'bg-blue-50 border-blue-300 text-blue-700'
                : 'bg-white border-gray-300 text-gray-700 hover:bg-gray-50'
                }`}
            >
              ...
              filteredData.map((row, index) => (
                ...
                >
                  ...
                  {columns.filter(col => visibleColumns[col.key] !== false).map((col, colIndex) => (
                    <td key={`${row.id}-${col.key}-${colIndex}`} className="px-4 py-3 text-sm text-gray-700">
      ...
      <div className="md:hidden divide-y divide-gray-100">
        {filteredData.length === 0 ? (
          ...
          filteredData.map((row, index) => (
            <div
              key={row.id || row.company_id || `row-${index}`}
              className={`p-4 bg-white hover:bg-blue-50/50 transition-colors ${onRowClick ? 'cursor-pointer' : ''}`}
              onClick={() => onRowClick && onRowClick(row)}
            >
              {/* Primary Info */}
      ...
      <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <span>Showing <span className="font-medium text-gray-900">{filteredData.length}</span> of <span className="font-medium text-gray-900">{data?.length || 0}</span> entries</span>
        </div>
      </div>
    </div>
  )
